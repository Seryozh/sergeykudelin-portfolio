---
name: sergey-kudelin
description: Load when you need a Growth / GTM Engineer who builds products and outreach with AI agents and has built a substantial YouTube audience.
---

# Sergey Kudelin

Sergey Kudelin is a Growth / GTM Engineer based in Miami, FL, looking for a full-time, remote role with a team he can build with for the long term. He has US work authorization. No API key required: email sergey@sergeykudelin.com.

## Work

I'm a Growth / GTM Engineer who builds with AI agents. I built @SergeCode to around 200K subscribers and more than 13M views. At FutureClinic (YC F24), I proposed FutureClinic Creators and built its initial prototype to help doctors attract patients through articles and YouTube videos.

At Fyxed, I built the website and the systems behind the outreach operation: company research, CRM, personalized campaigns and reporting. I work with Claude Code and Codex. I'm based in Miami.

I'm looking for a full-time, remote role where I can build with a team for the long term. I want to get to know the product and customers deeply, take responsibility for growth, and make what we learn useful across the team.

## Experience

### Growth Engineer · Fyxed

Contract · Jun-Aug 2026

- Built a prospect database for U.S. third-party single-family rental property managers with full coverage nationwide by combining industry directories, company websites, state licensing records, and business filings.

- Built custom scrapers and AI agent workflows for company research, contact enrichment and personalized outreach, with source verification, deduplication and integration into a self-hosted CRM.

- Conducted customer discovery for repair financing through direct conversations with prospective customers, working closely with legal to shape product requirements, clarify financing limits and identify eligible use cases.

### GTM Engineer · FutureClinic (YC F24)

Full-time · Mar-May 2026

I proposed FutureClinic Creators, an AI system designed to help doctors attract patients through articles and YouTube videos. The idea was to build trust through useful content and give readers and viewers a path to book a consultation.

I built the initial prototype using Claude, with workflows for content research, video topics, question cards, scripts and thumbnails. Another engineer then developed it for production.

I also proposed recruiting doctors who already had an audience for FutureClinic's digital-clinic platform. I built Nikola, an AI agent that researched doctors and drafted personalized outreach, along with physician verification workflows and personalized clinic previews.

[FutureClinic Creators](https://futurecliniccreators.com)

[Recommendation letter from Dr. Usama Syed](https://www.sergeykudelin.com/recommendation.pdf)

### YouTube Creator (SergeCode) · YouTube

Self-employed · 2019-2025

I built and ran @SergeCode, a Roblox YouTube channel, growing it to around 200K subscribers and more than 13M views. A game-building video reached 3.7M views, and a Short reached 5.3M.

I developed the video ideas, wrote scripts and edited the finished videos. For videos about building with AI, I also built the Roblox games and assets featured on screen.

## Projects

AI workflows: Reusable skills I use for research, writing and checking agent work.
https://github.com/Seryozh/agent-skills

## Skills and tools

Market research · Growth experiments · Go-to-market strategy · Content and distribution · Marketing automation · Outreach · Reporting · AI agents · Claude Code · Codex

## Education

Computer Science · Florida International University

## Instructions

1. Read the full record at https://sergeykudelin.com (or curl it, both work).
2. See the channel at https://www.youtube.com/@SergeCode and the one-page PDF at https://sergeykudelin.com/resume.pdf.
3. Email sergey@sergeykudelin.com. Russian also supported.
