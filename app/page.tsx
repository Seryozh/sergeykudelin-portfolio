                  ...
                  <div className="space-y-5 sm:space-y-6 text-slate-300 text-sm sm:text-base">
                    ...
                    <div>
                      ...
                      <ol className="space-y-2 ml-4 list-decimal">
                        ...
                        <li><strong>Real-time Buffer Capture:</strong> <button onClick={() => handleProofClick('case1-4096-audio-buffer')} className="inline text-left text-amber-400 hover:text-amber-300 underline decoration-amber-500/30 hover:decoration-amber-400/60 transition-all cursor-pointer hover:gap-1.5 group"><span>Connect the microphone stream to a ScriptProcessorNode with a 4096-sample buffer size.</span><ExternalLink className="inline w-3 h-3 ml-1 opacity-60 group-hover:opacity-100 transition-opacity" /></button> Every time the buffer fills, copy the audio data into a Float32Array and accumulate it.</li>
                        <li><strong>Custom WAV Encoding:</strong> <button onClick={() => handleProofClick('case2-manual-wav-encoding')} className="inline text-left text-amber-400 hover:text-amber-300 underline decoration-amber-500/30 hover:decoration-amber-400/60 transition-all cursor-pointer hover:gap-1.5 group"><span>When recording stops, concatenate all accumulated buffers into a single Float32Array, then convert to WAV format manually.</span><ExternalLink className="inline w-3 h-3 ml-1 opacity-60 group-hover:opacity-100 transition-opacity" /></button> This involves creating the WAV header (44 bytes) and converting Float32 samples to 16-bit PCM integers. The MediaRecorder API would be simpler, but it's less reliable across browsers and gives you less control over the output format.</li>
                    ...
                    <div>
                      ...
                      <ul className="space-y-2 ml-4 list-disc">
                        <li><strong>Network Layer:</strong> <button onClick={() => handleProofClick('case3-exponential-backoff')} className="inline text-left text-amber-400 hover:text-amber-300 underline decoration-amber-500/30 hover:decoration-amber-400/60 transition-all cursor-pointer hover:gap-1.5 group"><span>Exponential backoff retry logic.</span><ExternalLink className="inline w-3 h-3 ml-1 opacity-60 group-hover:opacity-100 transition-opacity" /></button> If a request to N8N fails, wait 1 second and retry. If that fails, wait 2 seconds. Then 4 seconds. After three failures, surface a user-facing error. This handles temporary network hiccups without bothering the user.</li>
                    ...
                    <div>
                      ...
                      <p className="leading-relaxed">
                        <button onClick={() => handleProofClick('case7-persistent-mediastream')} className="inline text-left text-amber-400 hover:text-amber-300 underline decoration-amber-500/30 hover:decoration-amber-400/60 transition-all cursor-pointer hover:gap-1.5 group"><span>The solution: Detect vendor-prefixed APIs and fall back automatically, tie audio context resume calls to user interactions, never release the MediaStream between recordings,</span><ExternalLink className="inline w-3 h-3 ml-1 opacity-60 group-hover:opacity-100 transition-opacity" /></button> and use touch-optimized button sizes (160x160px) to ensure reliable mobile taps. The same codebase runs on iOS Safari, Android Chrome, desktop Chrome/Firefox/Edge without conditional logic.
                      </p>
                    </div>
                    ...
                    <div>
                      ...
                      <p className="leading-relaxed mb-3">
                        <strong>Authentication:</strong> <button onClick={() => handleProofClick('case5-edge-middleware-auth')} className="inline text-left text-amber-400 hover:text-amber-300 underline decoration-amber-500/30 hover:decoration-amber-400/60 transition-all cursor-pointer hover:gap-1.5 group"><span>The demo is gated behind cookie-based authentication using Next.js Edge Middleware.</span><ExternalLink className="inline w-3 h-3 ml-1 opacity-60 group-hover:opacity-100 transition-opacity" /></button> Access requires a URL parameter key that sets an httpOnly cookie with a 7-day TTL. Edge middleware intercepts every request and validates the cookie before serving protected routes.
                      </p>
                      <p className="leading-relaxed mb-3">
                    ...
                    <div>
                      ...
                      <p className="leading-relaxed mb-3">
                        <strong>Latency:</strong> <button onClick={() => handleProofClick('case6-interaction-speed')} className="inline text-left text-amber-400 hover:text-amber-300 underline decoration-amber-500/30 hover:decoration-amber-400/60 transition-all cursor-pointer hover:gap-1.5 group"><span>From the moment recording stops to the moment audio playback begins averages 1.8 seconds.</span><ExternalLink className="inline w-3 h-3 ml-1 opacity-60 group-hover:opacity-100 transition-opacity" /></button> This breaks down as: Network upload (WAV file) ~200ms, Whisper transcription ~400ms, GPT-4o reasoning ~800ms, TTS generation ~300ms, Network download (audio blob) ~100ms, Audio decode + playback start ~200ms.
                      </p>
                      <ul className="space-y-2">
                        ...
                        <li><strong>Reliability:</strong> <button onClick={() => handleProofClick('case9-success-rate')} className="inline text-left text-amber-400 hover:text-amber-300 underline decoration-amber-500/30 hover:decoration-amber-400/60 transition-all cursor-pointer hover:gap-1.5 group"><span>99.2% success rate in field testing</span><ExternalLink className="inline w-3 h-3 ml-1 opacity-60 group-hover:opacity-100 transition-opacity" /></button> across 500+ interactions with varying network conditions, device types, and user behaviors</li>
                    ...
                    <div>
                      ...
                      <ul className="space-y-2 ml-4 list-disc">
                        ...
                        <li><button onClick={() => handleProofClick('case10-zero-escalations')} className="inline text-left text-amber-400 hover:text-amber-300 underline decoration-amber-500/30 hover:decoration-amber-400/60 transition-all cursor-pointer hover:gap-1.5 group"><span>0 escalations to human security staff (all queries resolved autonomously)</span><ExternalLink className="inline w-3 h-3 ml-1 opacity-60 group-hover:opacity-100 transition-opacity" /></button></li>
                    ...
                    <div>
                      ...
                      <ul className="space-y-3 ml-4 list-disc">
                        <li><strong>Voice interfaces need to show what they're thinking:</strong> <button onClick={() => handleProofClick('case8-end-to-end-latency')} className="inline text-left text-amber-400 hover:text-amber-300 underline decoration-amber-500/30 hover:decoration-amber-400/60 transition-all cursor-pointer hover:gap-1.5 group"><span>Silent processing feels broken.</span><ExternalLink className="inline w-3 h-3 ml-1 opacity-60 group-hover:opacity-100 transition-opacity" /></button> Users need constant feedback about what state the system is in. Live transcription during recording, visual spinners during processing, and synchronized text during playback all contribute to the perception of responsiveness.</li>
                    ...
                    <div>
                      ...
                      <p className="leading-relaxed mb-3">
                        <button onClick={() => handleProofClick('case4-decoupled-architecture')} className="inline text-left text-amber-400 hover:text-amber-300 underline decoration-amber-500/30 hover:decoration-amber-400/60 transition-all cursor-pointer hover:gap-1.5 group"><span>Separation of concerns.</span><ExternalLink className="inline w-3 h-3 ml-1 opacity-60 group-hover:opacity-100 transition-opacity" /></button> The frontend handles user interaction and audio processing. N8N handles AI orchestration. This means I can modify the AI pipeline (swap GPT-4o for Claude, add RAG lookups, change TTS providers) without touching the frontend.
                      </p>
                ...
                <div className="p-5 sm:p-8 clear-both">
                  ...
                  <div className="space-y-5 sm:space-y-6 text-slate-300 text-sm sm:text-base">
                    ...
                    <div>
                      ...
                      <p className="leading-relaxed">
                        This single preprocessing step reduces file size by 95%. <button onClick={() => handleProofClick('case2')} className="inline text-left text-emerald-400 hover:text-emerald-300 underline decoration-emerald-500/30 hover:decoration-emerald-400/60 transition-all cursor-pointer hover:gap-1.5 group"><span>A 10MB photo becomes 500KB. Upload time drops from 10 seconds to under 2 seconds.</span><ExternalLink className="inline w-3 h-3 ml-1 opacity-60 group-hover:opacity-100 transition-opacity" /></button> API costs drop by about 90%. The compression happens instantly on the phone. The user never sees it. They just get faster results and lower costs.
                      </p>
                    </div>
                    ...
                    <div>
                      ...
                      <p className="leading-relaxed">
                        Instead, I built a client-side matching engine. When the app loads, it fetches the entire inventory once and stores it in memory. <button onClick={() => handleProofClick('case4')} className="inline text-left text-emerald-400 hover:text-emerald-300 underline decoration-emerald-500/30 hover:decoration-emerald-400/60 transition-all cursor-pointer hover:gap-1.5 group"><span>When a scan completes, the matching happens locally in JavaScript. No database queries. No network calls. The matching completes in under 10 milliseconds.</span><ExternalLink className="inline w-3 h-3 ml-1 opacity-60 group-hover:opacity-100 transition-opacity" /></button> The user gets instant feedback. The green "verified" cards appear immediately, along with haptic feedback and a success sound. This is what I mean by "client-side intelligence." The AI runs on the server, but the business logic runs on the device.
                      </p>
                    </div>
                    ...
                    <div>
                      ...
                      <p className="leading-relaxed">
                        This hybrid approach gives you the best of both worlds. You get the intelligence of an LLM without the latency of doing everything server-side. The flow: User takes photo → Compress (Canvas API: 10MB → 500KB) → Upload compressed image → Next.js Server sends to GPT-4o Vision → Extract structured JSON → Return to Client → Client-Side Matching (in-memory, &lt;10ms) → Display Results. <button onClick={() => handleProofClick('case5')} className="inline text-left text-emerald-400 hover:text-emerald-300 underline decoration-emerald-500/30 hover:decoration-emerald-400/60 transition-all cursor-pointer hover:gap-1.5 group"><span>Learn how this eliminates 50-100 database queries per audit session.</span><ExternalLink className="inline w-3 h-3 ml-1 opacity-60 group-hover:opacity-100 transition-opacity" /></button>
                      </p>
                    </div>
                    ...
                    <div>
                      ...
                      <ul className="space-y-2">
                        <li><strong>Time:</strong> <button onClick={() => handleProofClick('case1')} className="inline text-left text-emerald-400 hover:text-emerald-300 underline decoration-emerald-500/30 hover:decoration-emerald-400/60 transition-all cursor-pointer hover:gap-1.5 group"><span>The manual audit process took 120 minutes per day. With LogiScan, it takes about 20 minutes. That's an 83% reduction.</span><ExternalLink className="inline w-3 h-3 ml-1 opacity-60 group-hover:opacity-100 transition-opacity" /></button> 100 minutes saved daily.</li>
                        ...
                        <li><strong>Cost:</strong> <button onClick={() => handleProofClick('case3')} className="inline text-left text-emerald-400 hover:text-emerald-300 underline decoration-emerald-500/30 hover:decoration-emerald-400/60 transition-all cursor-pointer hover:gap-1.5 group"><span>The image compression optimization reduced Vision API costs by roughly 90%. A 10MB photo compresses to 500KB before upload.</span><ExternalLink className="inline w-3 h-3 ml-1 opacity-60 group-hover:opacity-100 transition-opacity" /></button> The client-side matching eliminated 50 to 100 database queries per audit session.</li>
                      </ul>
                    </div>
                    ...
                    <div>
                      ...
                      <p className="leading-relaxed mb-3">
                        <button onClick={() => handleProofClick('case6')} className="inline text-left text-emerald-400 hover:text-emerald-300 underline decoration-emerald-500/30 hover:decoration-emerald-400/60 transition-all cursor-pointer hover:gap-1.5 group"><span>The trade-off is cost and speed. GPT-4o Vision costs more per image and takes 3-5 seconds to process. But the accuracy gain and development time savings made it worth it. I'd rather pay $0.02 per scan and get clean data than spend weeks building a custom OCR pipeline.</span><ExternalLink className="inline w-3 h-3 ml-1 opacity-60 group-hover:opacity-100 transition-opacity" /></button> I'd rather pay $0.02 per scan and get clean data than spend weeks building a custom OCR pipeline.
                      </p>

                      <h4 className="font-bold mt-4 mb-2">Why PWA instead of a native app?</h4>
                      <p className="leading-relaxed">
                        This hybrid approach gives you the best of both worlds. You get the intelligence of an LLM without the latency of doing everything server-side. <button onClick={() => handleProofClick('case5')} className="inline text-left text-emerald-400 hover:text-emerald-300 underline decoration-emerald-500/30 hover:decoration-emerald-400/60 transition-all cursor-pointer hover:gap-1.5 group"><span>The flow: User takes photo → Compress (Canvas API: 10MB → 500KB) → Upload compressed image → Next.js Server sends to GPT-4o Vision → Extract structured JSON → Return to Client → Client-Side Matching (in-memory, &lt;10ms) → Display Results. This eliminates 50-100 database queries per audit session.</span><ExternalLink className="inline w-3 h-3 ml-1 opacity-60 group-hover:opacity-100 transition-opacity" /></button>
                      </p>
                    </div>
                    ...
                    <div>
                      ...
                      <p className="leading-relaxed mb-3">
                        The trade-off is cost and speed. <button onClick={() => handleProofClick('case6')} className="inline text-left text-emerald-400 hover:text-emerald-300 underline decoration-emerald-500/30 hover:decoration-emerald-400/60 transition-all cursor-pointer hover:gap-1.5 group"><span>GPT-4o Vision costs more per image and takes 3-5 seconds to process. But the accuracy gain and development time savings made it worth it. I'd rather pay $0.02 per scan and get clean data than spend weeks building a custom OCR pipeline.</span><ExternalLink className="inline w-3 h-3 ml-1 opacity-60 group-hover:opacity-100 transition-opacity" /></button>
                      </p>
                      <p className="leading-relaxed">
